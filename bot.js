...
// Premium-лимит для «оживить фото / видео по фото» на 5-дневный период.
// Это НЕ бонусный накопительный кредит: лимит обновляется каждые 5 дней.
...
const RANDOM_NUDGE_MESSAGES = [
  ...
  "⏳ **Твои лимиты обновляются каждые 5 дней, всегда тебя ждём**",
...
function getMemoryLimitState(userId) {
  const key = getUserRequestKey(userId);
...
async function getUserLimitPeriodStart(userId) {
  const key = getUserRequestKey(userId);

  if (!dbPool) {
    return getMemoryLimitState(userId).periodStart;
  }
...
async function getUserLimitResetDateRu(userId) {
  const periodStart = await getUserLimitPeriodStart(userId);
  const endsAt = getLimitPeriodEndsAt(periodStart);

  if (!endsAt) return "через 5 дней";
...
// Получаем использование пользователя за текущий 5-дневный период.
async function getUserRequestCounts(userId) {
  const key = getUserRequestKey(userId);

  if (!dbPool) {
    const state = getMemoryLimitState(userId);
...
// Увеличиваем счётчик нужного типа в текущем 5-дневном периоде.
...
async function sendAiLimitReached(target, userId, type, userLimits) {
  const resetDate = await getUserLimitResetDateRu(userId);
...
async function getVideoAccessForUser(userId) {
  ...
  // Premium: лимит «оживить фото / видео по фото» действует 5 дней.
  // Это периодический лимит, а не накопительный бонусный кредит.
...
// Очищаем истёкшие 5-дневные периоды только для in-memory варианта.
...
async function sendFamilyVideoHelp(target, userId) {
  ...
  if (videoAccess.allowed) {
    ...
    const accessText =
      videoAccess.source === "premium"
        ? `У вас доступно Premium-видео в текущем 5-дневном периоде: **${videoAccess.premiumVideosLeft}**.`
  ...
  if (videoAccess.premium) {
    text += [
      "Ваше **Premium-видео в текущем 5-дневном периоде уже использовано**.",
...
async function sendPremiumInfo(target, userId) {
  ...
  let text =
    "💵 **Купить Премиум**\n\n" +
    "*Что дает Премиум?*\n\n" +
    `**1️⃣ ${PREMIUM_IMAGE_REQUEST_LIMIT} фото каждые 5 дней с лучшей моделью.**\n` +
    `**2️⃣ ChatGPT ${PREMIUM_CHATGPT_REQUEST_LIMIT} запросов каждые 5 дней.**\n` +
    `**3️⃣ Оживить фото: ${PREMIUM_VIDEO_REQUEST_LIMIT} раз каждые 5 дней.**\n` +
...
const prompt = shouldApplyPhotoFormat
  ...
  // Проверка лимита по картинкам за текущий 5-дневный период
  ...
  if (videoAccess.source === "premium") {
    ...
    await sendMaxMessage(
      ...
      [
        ...
        "",
        `Premium-видео в текущем 5-дневном периоде осталось: **${premiumVideosLeft}**.`,
        "",
        premiumVideosLeft > 0
          ? "Можете создать ещё одно Premium-видео в текущем периоде."
        ...
        if (result.granted && result.userId) {
          ...
          await sendMaxMessage(
            ...
            [
              ...
              `• ${PREMIUM_IMAGE_REQUEST_LIMIT} фото каждые 5 дней с лучшей моделью;`,
              `• ${PREMIUM_CHATGPT_REQUEST_LIMIT} запросов ChatGPT каждые 5 дней;`,
              `• оживить фото / видео по фото: ${PREMIUM_VIDEO_REQUEST_LIMIT} раз каждые 5 дней;`,
