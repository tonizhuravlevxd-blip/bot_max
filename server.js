...
const LIMIT_RESET_DAYS = Number(process.env.LIMIT_RESET_DAYS || 5);
...
const MENU_EARN_ENTRY_PAYLOAD = "menu_earn_entry";
const EARN_CONSENT_YES_PAYLOAD = "earn_consent_yes";
const EARN_CONSENT_NO_PAYLOAD = "earn_consent_no";
...
// Бесплатные лимиты: отдельный 5-дневный период для каждого пользователя.
const userRequestCounts = {};
// Premium-лимиты: отдельный ежедневный счётчик, чтобы они не смешивались
// с бесплатным 5-дневным периодом.
const premiumDailyRequestCounts = {};
...
function getUserUsername(update) {
  const candidates = [
...
async function initLimitsDb() {
  ...
  // Premium хранится отдельно, чтобы ежедневные счётчики не смешивались
  // с бесплатным 5-дневным периодом в max_bot_limits.
  await dbPool.query(`
    CREATE TABLE IF NOT EXISTS max_bot_premium_daily_limits (
      user_id TEXT NOT NULL,
      bot_key TEXT NOT NULL,
      date DATE NOT NULL,
      images INTEGER NOT NULL DEFAULT 0,
  ...
  await dbPool.query(`
    CREATE INDEX IF NOT EXISTS idx_max_bot_premium_daily_limits_user_date
    ON max_bot_premium_daily_limits (user_id, date)
  `);

  await dbPool.query(`
    CREATE INDEX IF NOT EXISTS idx_max_bot_premium_daily_limits_bot_key
    ON max_bot_premium_daily_limits (bot_key)
  `);

  console.log("Limits DB initialized: free=5 days, Premium=daily");
}
...
const RANDOM_NUDGE_MESSAGES = [
  ...
  "⏳ **Бесплатные лимиты обновляются каждые 5 дней, а Premium — каждый день. Всегда тебя ждём**",
...
function getTodayDate() {
  // Формат YYYY-MM-DD в UTC, как и раньше.
  return new Date().toISOString().slice(0, 10);
}

function normalizeLimitPeriodDate(value) {
  if (!value) return "";
...
function getLimitPeriodEndsAt(periodStart) {
  const cleanDate = normalizeLimitPeriodDate(periodStart);
  const startedAt = Date.parse(`${cleanDate}T00:00:00.000Z`);

  if (!cleanDate || Number.isNaN(startedAt)) {
    return 0;
  }

  return startedAt + LIMIT_RESET_DAYS * LIMIT_DAY_MS;
}

function isLimitPeriodActive(periodStart, now = Date.now()) {
  const endsAt = getLimitPeriodEndsAt(periodStart);
  return Boolean(endsAt && now < endsAt);
}

function getFreeMemoryLimitState(userId) {
  const key = getUserRequestKey(userId);
  let state = userRequestCounts[key];

  if (!state || !isLimitPeriodActive(state.periodStart)) {
    state = {
    ...
    userRequestCounts[key] = state;
  }

  return state;
}

function getPremiumDailyDate() {
  // Premium обновляется по календарному дню Москвы, в 00:00 МСК.
  return getMoscowDateYmd();
}

function getPremiumDailyMemoryLimitState(userId) {
  const key = getUserRequestKey(userId);
  const today = getPremiumDailyDate();
  let state = premiumDailyRequestCounts[key];
  ...
  if (!state || state.date !== today) {
    ...
    premiumDailyRequestCounts[key] = state;
  }

  return state;
}

async function getFreeUserLimitPeriodStart(userId) {
  const key = getUserRequestKey(userId);

  if (!dbPool) {
    return getFreeMemoryLimitState(userId).periodStart;
  }
  ...
  const latestPeriodStart = normalizeLimitPeriodDate(result.rows[0]?.date);

  if (latestPeriodStart && isLimitPeriodActive(latestPeriodStart)) {
...
function getPremiumDailyResetEndsAt() {
  const today = getPremiumDailyDate();
  ...
  // Следующие 00:00 МСК = 21:00 UTC предыдущего календарного дня.
  return Date.UTC(year, month - 1, day + 1, -3, 0, 0);
}

async function getUserLimitResetDateRu(userId, premiumOverride = null) {
  const premium =
  ...
  if (premium) {
    endsAt = getPremiumDailyResetEndsAt();
  } else {
    const periodStart = await getFreeUserLimitPeriodStart(userId);
    endsAt = getLimitPeriodEndsAt(periodStart);
  }

  if (!endsAt) {
    return premium ? "завтра" : `через ${LIMIT_RESET_DAYS} дней`;
  }
...
// Бесплатные пользователи: один счётчик на 5 дней.
// Premium-пользователи: отдельный счётчик на каждый день.
async function getUserRequestCounts(userId, premiumOverride = null) {
  const key = getUserRequestKey(userId);
  const premium =
    typeof premiumOverride === "boolean"
      ? premiumOverride
  ...
  if (!dbPool) {
    const state = premium
      ? getPremiumDailyMemoryLimitState(userId)
      : getFreeMemoryLimitState(userId);

    return {
      images: Number(state.images) || 0,
      chatgpt: Number(state.chatgpt) || 0,
      videos: Number(state.videos) || 0
    };
  }

  const table = premium ? PREMIUM_DAILY_LIMITS_TABLE : LIMITS_TABLE;
  const usageDate = premium
    ? getPremiumDailyDate()
    : await getFreeUserLimitPeriodStart(userId);
...
// Увеличиваем счётчик в правильном периоде:
// бесплатно — 5 дней, Premium — текущий день.
async function incrementRequestCount(userId, type, premiumOverride = null) {
  const key = getUserRequestKey(userId);
  ...
  await dbPool.query(
    `
      INSERT INTO ${table} (user_id, bot_key, date, ${col})
      VALUES ($1, $2, $3, 1)
      ON CONFLICT (user_id, bot_key, date)
      DO UPDATE SET ${col} = ${table}.${col} + 1
    `,
    [key, BOT_KEY, usageDate]
  );
}
...
function buildLimitReachedAttachments(userId, premium) {
  const buttons = [];
...
async function sendAiLimitReached(target, userId, type, userLimits) {
  const resetDate = await getUserLimitResetDateRu(userId, userLimits.premium);
  const limit = Number(userLimits?.[type] || 0);
  ...
  const text = userLimits.premium
    ? [
        "🥱 **Premium-лимит закончился**",
        "",
        `Использовано: **${limit}/${limit}** — ${featureName}.`,
        `Лимит обновится **${resetDate}**.`,
        "",
        "После обновления можно будет продолжить."
      ].join("\n")
    : [
        "🥱 **Бесплатный лимит закончился**",
        "",
        `Использовано: **${limit}/${limit}** — ${featureName}.`,
        `Бесплатный лимит обновится **${resetDate}**.`,
        "",
        `Чтобы продолжить сейчас, купите Премиум за **${Number(PREMIUM_PRICE_RUB).toFixed(0)} ₽**.`
      ].join("\n");

  return sendMaxMessageWithAttachments(
    target,
    text,
    buildLimitReachedAttachments(userId, userLimits.premium)
  );
}
...
async function getVideoAccessForUser(userId) {
  ...
  // Premium: лимит «оживить фото / видео по фото» обновляется каждый день.
  // Это ежедневный лимит, а не накопительный бонусный кредит.
...
// Очищаем истёкшие in-memory счётчики:
// бесплатные — после 5 дней, Premium — при наступлении нового дня.
function resetDailyLimits() {
  setInterval(() => {
    if (!dbPool) {
      Object.entries(userRequestCounts).forEach(([key, state]) => {
        if (!isLimitPeriodActive(state?.periodStart)) {
      ...
      Object.entries(premiumDailyRequestCounts).forEach(([key, state]) => {
        if (state?.date !== today) {
          delete premiumDailyRequestCounts[key];
        }
      });
    }
  }, 60 * 60 * 1000).unref?.();
    ...
    [
      {
        type: "callback",
        text: "💰Заработать",
        payload: MENU_EARN_ENTRY_PAYLOAD
      }
    ],
    [
      {
...
async function sendEarnConsent(target) {
  const text = [
    "💰 **Участие в программе «Заработать»**",
    "",
    "Нажимая **«ДА»**, вы подтверждаете, что ознакомились и согласны с:",
    "• [Офертой](https://disk.yandex.ru/i/8Z6BsYfupgMq1Q);",
    "• [Политикой персональных данных](https://disk.yandex.ru/i/LHakrABNtGiVMw).",
    "",
    "Также вы принимаете правила реферальной программы:",
    "• вознаграждение начисляется только за активных приглашённых пользователей;",
    "• самостоятельные регистрации, накрутка и фиктивная активность не учитываются;",
    "• заявки на вывод могут проходить проверку;",
    "• при нарушении правил начисления и выплата могут быть отменены.",
    "",
    "Вы принимаете условия участия?"
  ].join("\n");
          ...
          [
            {
              type: "callback",
              text: "✅ ДА, принимаю",
              payload: EARN_CONSENT_YES_PAYLOAD
            }
          ],
          [
            {
              type: "callback",
              text: "❌ НЕТ",
              payload: EARN_CONSENT_NO_PAYLOAD
            }
          ]
        ]
      }
...
async function sendPremiumInfo(target, userId) {
  ...
  let text =
    "💵 **Купить Премиум**\n\n" +
    "*Что дает Премиум?*\n\n" +
    `**1️⃣ ${PREMIUM_IMAGE_REQUEST_LIMIT} фото каждый день с лучшей моделью.**\n` +
    `**2️⃣ ChatGPT ${PREMIUM_CHATGPT_REQUEST_LIMIT} запросов каждый день.**\n` +
    `**3️⃣ Оживить фото: ${PREMIUM_VIDEO_REQUEST_LIMIT} раз каждый день.**\n` +
...
const PREMIUM_DAILY_LIMITS_TABLE = "max_bot_premium_daily_limits";
      ...
      input: [
        {
          role: "system",
          content:
            "Ты полезный ассистент внутри мессенджера MAX. Отвечай кратко, ясно и по делу. Если вопрос требует пошагового ответа, структурируй ответ простыми абзацами. Используй смайлики в ответах. Если пользователь просит промт, промпт, prompt или promt — верни только сам готовый промт без объяснений, без вступления, без заголовков и без фразы «вот промт». Сам промт должен быть внутри двойных кавычек. Пример формата: \"cinematic portrait, soft light, realistic details\". Твой создатель — Бот SOSai. Алёна Журавлёва и Даша Журавлёва являются сёстрами создателя Бота SOSai, но не являются создателями бота."
        },
        ...history,
        {
          role: "user",
...
const prompt = shouldApplyPhotoFormat
  ...
  // Проверка лимита по картинкам: бесплатно за 5 дней, Premium за текущий день
  if (await isRequestLimitReached(userId, "images", userLimits.images)) {
    await sendAiLimitReached(target, userId, "images", userLimits);
    return;
  }
...
if (updateType === "bot_started") {
  ...
  // сначала показываем условия участия в программе «Заработать»
  if (String(startPayload || "").trim() === EARN_START_PAYLOAD) {
    clearUserImageMode(userId);
    clearFamilyVideoDraft(userId);
    clearHoroscopeSetupState(userId);

    await sendEarnConsent(target);
    return;
  }
...
if (callbackPayload === MENU_CREATE_VIDEO_PAYLOAD) {
  ...
  return;
      ...
      // Первичный вход из главного меню: сначала показываем условия
      if (callbackPayload === MENU_EARN_ENTRY_PAYLOAD) {
        clearUserImageMode(userId);
        clearFamilyVideoDraft(userId);
        clearHoroscopeSetupState(userId);
      ...
      // Пользователь принял условия — открываем раздел «Заработать»
      if (callbackPayload === EARN_CONSENT_YES_PAYLOAD) {
        clearUserImageMode(userId);
        clearFamilyVideoDraft(userId);
        clearHoroscopeSetupState(userId);
      ...
      // Пользователь не принял условия — раздел не открываем
      if (callbackPayload === EARN_CONSENT_NO_PAYLOAD) {
        clearUserImageMode(userId);
        clearFamilyVideoDraft(userId);
        clearHoroscopeSetupState(userId);
        ...
        await sendMainMenu(
          target,
          "❌ Вы не приняли условия программы «Заработать». Раздел не был открыт.\n\nВыберите другую функцию:"
        );
        return;
      }

      // Обновление статистики внутри уже открытого раздела — без повторного согласия
...
if (isVideoRequest(userText, Boolean(incomingImageUrl))) {
  ...
  return;
    ...
    if (await isRequestLimitReached(userId, "chatgpt", userLimits.chatgpt)) {
      await sendAiLimitReached(target, userId, "chatgpt", userLimits);
      return;
    }
        ...
        if (result.granted && result.userId) {
          ...
          await sendMaxMessage(
            ...
            [
              ...
              `• ${PREMIUM_IMAGE_REQUEST_LIMIT} фото каждый день с лучшей моделью;`,
              `• ${PREMIUM_CHATGPT_REQUEST_LIMIT} запросов ChatGPT каждый день;`,
              `• оживить фото / видео по фото: ${PREMIUM_VIDEO_REQUEST_LIMIT} раз каждый день;`,
